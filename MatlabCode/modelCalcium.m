...
if ~isfield(S,'spkTimes') || ~S.recycleSpikeTimes % if (spkTimes hasn't been set) or (not to use recycleSpikeTimes)
    S.spkTimes = cell(doVectorized,1); % create spkTimes cell
    for n = 1:doVectorized
        %     poisson spike train
        S.spkTimes{n,1} = PoissonSpikeTrain(S.spikeRate(n),S.dur+S.offset);
    end
else
    if ~iscell(S.spkTimes) % is spkTimes is not a cell: make it a cell
        spkTimes = S.spkTimes;
        S.spkTimes = cell(1,1);
        S.spkTimes{1} = spkTimes;
    end
    if length(S.spkTimes) ~= doVectorized % ~=:not equal to
